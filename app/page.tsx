...
import {
  ...
  User, ShieldCheck, PenTool, Eraser, Lasso, RotateCcw, CheckCircle,
  ...
  Move, FileQuestion, X, Eye, Clipboard
} from 'lucide-react';
...
// ==========================================
// 2. 2022 개정 교육과정 전체 데이터 구조 (공통/선택 구분 보완)
// ==========================================
...
export default function SmartMathApp() {
  ...
  const [viewMode, setViewMode] = useState<
    ...
    'register_manual' | 'print' | 'assignment_manage' | 'register_ai'
  >('dashboard');
  ...
  const [studentTab, setStudentTab] = useState<'practice' | 'assignment'>('practice');
  ...
  const [problems, setProblems] = useState<Problem[]>([
    {
      ...
      type: 'multiple',
      title: '다항식 연산 예제',
      content: '다항식 $A=x^{2}+2x-1$, $B=2x^{2}-x+3$ 일 때 2A + B를 간단히 하시오.',
      difficulty: '중',
    ...
    },
    {
      ...
      content: '다항식 (x + 2)(x²-2x+4)를 전개하였을 때 x³의 계수와 상수항의 합을 구하시오.',
      difficulty: '중',
      options: [],
      answer: '9',
      explanation: '전개식은 x³+ 8이므로 x³의 계수는 1, 상수항은 8입니다. 따라서 합은 9입니다.',
  ...
  const [manualContentImage, setManualContentImage] = useState<string>('');
  const [manualAnswer, setManualAnswer] = useState('');
  const [manualExplanation, setManualExplanation] = useState('');
  const [manualExplanationImage, setManualExplanationImage] = useState<string>('');
  ...
  const step2Options = useMemo(() => step1 ? Object.keys(curriculumData[selectedLevel]?.[step1] || {}) : [], [selectedLevel, step1]);
  ...
  const handleAssignExamToStudents = (exam: SavedExamPaper) => {
    ...
    alert(`'${exam.title}' 시험지가 학생 과제로 부과되었습니다!`);
  };
  ...
  const readClipboardImage = async (): Promise<string | null> => {
    try {
      if (!navigator.clipboard || !navigator.clipboard.read) {
        alert('이 브라우저 혹은 환경에서는 클립보드 직접 읽기를 지원하지 않습니다. 단축키 Ctrl+V를 사용해 주세요.');
        return null;
      }
      ...
      console.error(err);
      alert('클립보드 권한 거부 또는 이미지를 읽을 수 없습니다. 영역 클릭 후 Ctrl+V로 붙여넣어 주세요.');
    }
    return null;
  };
  ...
  const handleManualSubmit = (e: React.FormEvent) => {
    ...
    const newProblem: Problem = {
      ...
      contentImage: manualContentImage,
      difficulty: manualDifficulty,
      options: [],
      answer: manualAnswer,
      explanation: manualExplanation,
      explanationImage: manualExplanationImage,
  ...
  // AI 분리 캡처 처리 (수정본: API 전송 Payload 및 내부 변수 오류 수정 완료)
  const processMultiSegmentationAI = async () => {
    if (!aiProblemImage && !aiAnswerImage) {
      alert('문제 캡처 이미지 또는 답지 캡처 이미지를 붙여넣어 주세요.');
      return;
    }
    setIsAnalyzing(true);
    try {
  ...
  const deleteSelectedStrokes = (probId: string) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
      {/* PWA 설치 배너 */}
      {showInstallBanner && (
        <div className="bg-gradient-to-r from-indigo-700 via-purple-700 to-indigo-800 text-white px-4 py-3 shadow-xl flex items-center justify-between z-50 relative border-b border-indigo-500">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black px-3 py-1.5 rounded-lg text-xs flex items-center space-x-1 shadow"
            >
      ...
      {/* 헤더 */}
      <header className="sticky top-0 z-50 border-b border-slate-800 bg-slate-900/90 backdrop-blur-md px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center space-x-3">
        ...
        <div className="flex bg-slate-800/80 p-1 rounded-xl border border-slate-700">
          <button
            onClick={() => { setRoleMode('teacher'); setStudentSolving(false); }}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          ...
          <>
            <div className="flex flex-wrap gap-2 mb-6 bg-slate-900 p-2 rounded-xl border border-slate-800">
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-xs font-semibold ${
              ...
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-xs font-semibold ${
                  viewMode === 'generate_exam' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                <FileSpreadsheet className="w-4 h-4" />
                <span>자율 맞춤 시험지 출제</span>
              </button>

              <button
                onClick={() => setViewMode('saved_exams')}
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-xs font-semibold ${
                  viewMode === 'saved_exams' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-xs font-semibold ${
                  viewMode === 'register_manual' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                <Edit3 className="w-4 h-4" />
                <span>수동 문제 제작 (객관식/주관식 분리 & 캡처)</span>
              </button>

              <button
                onClick={() => setViewMode('assignment_manage')}
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-xs font-semibold ${
                  viewMode === 'assignment_manage' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                <ClipboardList className="w-4 h-4" />
                <span>학생 인가 및 과제 제출 대시보드</span>
              </button>

              <button
                onClick={() => setViewMode('register_ai')}
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-xs font-semibold ${
                  viewMode === 'register_ai' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                <Sparkles className="w-4 h-4" />
                <span>AI 캡처 자동 추출 (문제&답지 분리)</span>
              </button>

              <button
                onClick={() => setViewMode('print')}
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-xs font-semibold ${
                  viewMode === 'print' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                <Printer className="w-4 h-4" />
                <span>시험지 인쇄 (PDF/한글)</span>
              </button>
            </div>

            {/* 교육과정 및 모의고사 선택기 */}
            <div className="mb-8 p-6 bg-slate-900/60 rounded-2xl border border-slate-800">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 bg-rose-950/20 p-4 rounded-xl border border-rose-900/40">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {EXAM_YEARS.map(yr => <option key={yr} value={yr}>{yr} 기출</option>)}
                    </select>
                  </div>
                  <div>
            ...
            {viewMode === 'dashboard' && (
              <div className="space-y-4">
                <h2 className="text-lg font-bold flex items-center space-x-2">
                  ...
                  <span>등록된 문제 목록 ({problems.length})</span>
                </h2>
                    ...
                    <div key={prob.id} className="p-6 bg-slate-900/80 border border-slate-800 rounded-2xl shadow-lg">
                      ...
                      {prob.contentImage ? (
                        <div className="mt-3 p-2 bg-slate-950 rounded-xl border border-slate-800">
                          <img src={prob.contentImage} alt="문제지문+보기" className="max-h-96 rounded object-contain" />
                        </div>
                      ) : (
                      ...
                      {prob.type === 'multiple' && prob.options.length > 0 && (
            ...
            {viewMode === 'register_manual' && (
              <div className="p-8 bg-slate-900/80 border border-slate-800 rounded-2xl shadow-xl">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="px-3 py-1 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-lg flex items-center gap-1 shadow transition-all"
                      >
                      ...
                      <div className="flex items-center justify-between text-xs text-slate-400">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-bold rounded-md flex items-center gap-1 shadow"
                          >
              ...
              <div className="p-8 bg-gradient-to-br from-indigo-950/40 via-slate-900 to-slate-900 border border-indigo-500/30 rounded-2xl shadow-2xl space-y-6">
                ...
                {isAnalyzing ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-indigo-300">[1단계] 문제 이미지 선택</span>
                        <button
                          type="button"
                          onClick={async () => {
                            const img = await readClipboardImage();
                      ...
                      <div
                        ...
                        className="p-6 bg-slate-950/60 border-2 border-dashed border-indigo-500/40 hover:border-indigo-500 rounded-xl text-center space-y-3 focus:outline-none focus:border-indigo-400 flex-1 flex flex-col justify-center"
                      >
                        <div className="flex items-center justify-center space-x-2 text-indigo-400 font-bold text-xs">
                          <ImageIcon className="w-5 h-5" />
                          <span>문제 캡처 영역 (Ctrl+V)</span>
                        </div>
                        <p className="text-[11px] text-slate-400">
                          여러 객관식/주관식 문제가 들어있는 영역을 캡처한 후 클릭하여 <strong className="text-indigo-300">Ctrl+V</strong> 또는 위의 버튼을 누르세요.
                        </p>
                        {aiProblemImage ? (
                    ...
                    <div className="flex flex-col space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-emerald-300">[2단계] 정답/해설 이미지 선택</span>
                        <button
                          type="button"
                          onClick={async () => {
                            const img = await readClipboardImage();
                          ...
                          className="px-3 py-1 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-lg flex items-center gap-1 shadow transition-all"
                        >
                          <Clipboard className="w-3.5 h-3.5" />
                          <span>클립보드 붙여넣기</span>
                        </button>
                      ...
                      <div
                        ...
                        className="p-6 bg-slate-950/60 border-2 border-dashed border-emerald-500/40 hover:border-emerald-500 rounded-xl text-center space-y-3 focus:outline-none focus:border-emerald-400 flex-1 flex flex-col justify-center"
                      >
                        <div className="flex items-center justify-center space-x-2 text-emerald-400 font-bold text-xs">
                          <CheckCircle2 className="w-5 h-5" />
                          <span>정답/해설 캡처 영역 (Ctrl+V)</span>
                        </div>
                        <p className="text-[11px] text-slate-400">
                          정답 및 상세해설이 들어있는 영역을 캡처한 후 클릭하여 <strong className="text-emerald-300">Ctrl+V</strong> 또는 위의 버튼을 누르세요.
                        </p>
                        {aiAnswerImage ? (
                          <div className="relative pt-2">
                ...
                {!isAnalyzing && (
                  <button
                    ...
                    className="w-full py-3.5 bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-600 hover:from-indigo-500 hover:to-indigo-500 text-white font-bold rounded-xl text-xs flex items-center justify-center space-x-2 shadow-xl"
                  >
                    <Sparkles className="w-4 h-4" />
            ...
            {viewMode === 'saved_exams' && (
              <div className="p-8 bg-slate-900/80 border border-slate-800 rounded-2xl shadow-xl space-y-6">
                <div className="border-b border-slate-800 pb-4">
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    자율 출제로 구성한 시험지를 보관하고, 지정한 마감일로 학생 과제로 즉시 전송합니다.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-6 bg-slate-900/80 border border-slate-800 rounded-2xl shadow-xl space-y-4">
                  <h3 className="text-base font-bold text-white flex items-center gap-2">
                    <User className="w-5 h-5 text-indigo-400" /> 학생 인가 등록 (비밀번호 지정)
                  </h3>
                  <div className="space-y-3">
                    <input
                      type="text"
                  ...
                  <div className="pt-2 border-t border-slate-800 divide-y divide-slate-800 text-xs">
                    {allowedStudents.map(st => (
                      <div key={st.id} className="py-2 flex justify-between items-center text-slate-300">
                        <span>{st.name}</span>
                ...
                <div className="p-6 bg-slate-900/80 border border-slate-800 rounded-2xl shadow-xl space-y-4">
                  <h3 className="text-base font-bold text-white flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-indigo-400" /> 과제 부과 & 실시간 제출 현황 & 마감 기간 수정
                  </h3>
                    ...
                    {assignments.map(asg => (
                      <div key={asg.id} className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-3">
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="text-xs font-bold text-indigo-300 block">{asg.title}</span>
                            <span className="text-[10px] text-slate-500">지정 과제 ID: {asg.id}</span>
                          </div>

                          {/* 과제 마감 기간 수정 기능 */}
                          <div className="flex items-center gap-1.5 bg-slate-900 p-1 rounded-lg border border-slate-800">
                            <span className="text-[10px] text-slate-400">마감일:</span>
                            <input
                              type="date"
                              value={asg.dueDate}
                              onChange={(e) => handleUpdateAssignmentDueDate(asg.id, e.target.value)}
                              className="bg-slate-950 border border-slate-700 rounded px-1.5 py-0.5 text-[11px] text-emerald-400"
                            />
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5 pt-2 border-t border-slate-800/60">
                          {allowedStudents.map(st => {
                            ...
                            const targetProbs = problems.filter(p => asg.problemIds.includes(p.id));

                            return (
                              <div key={st.id} className="flex justify-between items-center text-[11px] bg-slate-900/40 p-2 rounded-lg">
                                <span className="text-slate-300 font-medium">{st.name}</span>
                                {isSubmitted ? (
                                  <div className="flex items-center gap-2">
                                    <span className="text-emerald-400 font-bold flex items-center gap-1">
                                      <CheckCircle2 className="w-3 h-3" />
                                      제출완료 ({subRecord.score ?? 100}점)
                                    </span>
                                    <button
                                      onClick={() => setViewingSubmission({
                                        ...
                                        record: subRecord,
                                        problems: targetProbs
                                      })}
                                      className="px-2 py-0.5 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-bold rounded flex items-center gap-1 shadow"
                                    >
                                      <Eye className="w-3 h-3" /> 결과/해설 보기
                                    </button>
                                  </div>
                                ) : (
                                  <span className="text-rose-400 font-medium flex items-center gap-1">
                                    <Clock className="w-3 h-3" /> 미제출 (기간변경가능)
                                  </span>
                                )}
                              </div>
                            );
              ...
              <div className="space-y-4">
                <div className="flex gap-3">
                  <button
                    ...
                    className="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg flex items-center gap-2"
                  >
                    <Printer className="w-4 h-4" /> PDF 인쇄
                  </button>
                  <button
                    onClick={() => alert('한글(hwp) 호환 문서로 인쇄를 준비합니다.')}
                    className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-lg flex items-center gap-2"
                  >
                    <FileText className="w-4 h-4" /> 한글(hwp) 인쇄
                  </button>
                </div>

                <div className="bg-white text-black p-10 rounded-2xl shadow-2xl min-h-[800px]">
                  <div className="border-b-2 border-black pb-4 mb-8 text-center space-y-1">
                    <h1 className="text-2xl font-black">{examTitle}</h1>
                    <p className="text-xs text-slate-600">유캔&청운수학스마트문제은행</p>
                  </div>

                  <div className="grid grid-cols-2 gap-8 divide-x divide-slate-200">
                    {problems.map((p, idx) => (
                      <div key={p.id} className="pr-4 space-y-2 text-xs">
                        <div className="font-bold">{idx + 1}. [{p.type === 'subjective' ? '주관식' : '객관식'}] {p.title}</div>
                        {p.contentImage ? (
                          <img src={p.contentImage} alt="문제" className="max-h-48" />
                        ) : (
                          <div className="whitespace-pre-wrap">{p.content}</div>
                        )}
                        {p.type === 'multiple' && p.options.length > 0 && (
                          <div className="space-y-1 pl-2">
                            {p.options.map((opt, oIdx) => <div key={oIdx}>{numIcons[oIdx]} {opt}</div>)}
                          </div>
                        )}
                      </div>
                    ))}
        ...
        {roleMode === 'student' && (
          <div>
            {!isAuthenticated ? (
              <div className="p-8 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl max-w-md mx-auto space-y-6 my-12">
                <div className="text-center space-y-2">
                  ...
                  <h2 className="text-xl font-bold text-white">학생 비밀번호 인증</h2>
                  <p className="text-xs text-slate-400">선생님이 승인한 이름과 전화번호 뒤 4자리를 입력하세요.</p>
                </div>
                ...
                <form onSubmit={handleStudentLogin} className="space-y-4">
                  ...
                  <button
                    type="submit"
                    className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs transition-all shadow-lg"
                  >
                    인증 및 입장하기
                  </button>
                </form>
              ...
              <div className="p-8 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl max-w-3xl mx-auto space-y-6">
                <div className="flex justify-between items-center pb-4 border-b border-slate-800">
                  <div>
                    <h2 className="text-lg font-bold text-white">{currentStudent?.name} 학생 환영합니다!</h2>
                    <p className="text-xs text-slate-400">자율 수학 문제풀이 및 부여된 과제를 수행할 수 있습니다.</p>
                  </div>
                  <button
                ...
                <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800">
                  <button
                    ...
                    className={`flex-1 py-2 text-xs font-bold rounded-lg ${
                      studentTab === 'practice' ? 'bg-emerald-600 text-white' : 'text-slate-400'
                    }`}
                  >
                    자율 맞춤 문제풀이
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2 text-xs font-bold rounded-lg ${
                      studentTab === 'assignment' ? 'bg-emerald-600 text-white' : 'text-slate-400'
                    }`}
                  >
                    부여된 과제함 ({assignments.length})
                  </button>
                </div>
                ...
                {studentTab === 'practice' ? (
                  <div className="space-y-5">
                    <div>
                      <label className="block text-xs text-slate-400 mb-2 font-semibold">1. 학교급 선택</label>
                      <div className="grid grid-cols-4 gap-2">
                        {['초등부', '중등부', '고등부', '모의고사 기출'].map(lvl => (
                          <button
                            key={lvl}
                            onClick={() => {
                    ...
                    {selectedLevel !== '모의고사 기출' ? (
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div>
                          <label className="block text-xs text-slate-400 mb-1">
                            {selectedLevel === '고등부' ? '과목' : '학년'}
                          </label>
                          <select
                            value={step1}
                            onChange={(e) => { setStep1(e.target.value); setStep2(''); setStep3(''); }}
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-xs text-white"
                          >
                            <option value="">전체 과목/학년</option>
                            {step1Options.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                          </select>
                        </div>
                        <div>
                          ...
                          <select
                            value={step2}
                            disabled={!step1}
                            onChange={(e) => { setStep2(e.target.value); setStep3(''); }}
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-xs text-white disabled:opacity-40"
                          >
                            <option value="">전체 대단원/학기</option>
                            {step2Options.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                          </select>
                        </div>
                        <div>
                          ...
                          <select
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-xs text-white disabled:opacity-40"
                          >
                            <option value="">전체 중단원</option>
                            {step3Options.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                          </select>
                        </div>
                      </div>
                    ) : (
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 bg-rose-950/20 p-4 rounded-xl border border-rose-900/40">
                        <div>
                          <label className="block text-xs text-rose-300 font-bold mb-1">기출 학년</label>
                          <select
                            value={mockGrade}
                        ...
                        <div>
                          <label className="block text-xs text-rose-300 font-bold mb-1">기출 연도</label>
                          <select
                            value={mockYear}
                            onChange={(e) => setMockYear(e.target.value)}
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-xs text-white"
                        ...
                        <div>
                          <label className="block text-xs text-rose-300 font-bold mb-1">기출 월</label>
                          <select
                            value={mockMonth}
                        ...
                        <div>
                          <label className="block text-xs text-rose-300 font-bold mb-1">공통/선택 구분</label>
                          <select
                            value={mockSubjectType}
                            onChange={(e) => setMockSubjectType(e.target.value as any)}
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-xs text-white"
                          >
                            <option value="">전체</option>
                            <option value="공통과목">공통과목</option>
                            <option value="선택과목">선택과목</option>
                          </select>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-4">
                      <label className="block text-xs text-indigo-400 font-bold">2. 문제 유형 및 문제 수 지정</label>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="flex items-center justify-between bg-slate-900 p-3 rounded-lg border border-slate-800">
                          <span className="text-xs font-semibold text-slate-300">객관식 문항 수</span>
                          <input
                            type="number"
                            min={0}
                            max={50}
                        ...
                        <div className="flex items-center justify-between bg-slate-900 p-3 rounded-lg border border-slate-800">
                          <span className="text-xs font-semibold text-slate-300">주관식 문항 수</span>
                          <input
                            type="number"
                            min={0}
                            max={50}
                    ...
                    <button
                      ...
                      className="w-full py-3.5 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-black rounded-xl text-sm transition-all flex items-center justify-center space-x-2 shadow-lg"
                    >
                      <BookOpen className="w-5 h-5" />
                      <span>맞춤 문제 생성 & 풀이 시작</span>
                    </button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {assignments.map(asg => {
                      ...
                      const isDone = !!submissions[subKey];

                      return (
                        <div key={asg.id} className="p-5 bg-slate-950 rounded-2xl border border-slate-800 flex justify-between items-center">
                          <div>
                            <span className="text-xs text-indigo-400 font-bold block mb-1">마감일: ~{asg.dueDate}</span>
                            <h3 className="text-sm font-bold text-white">{asg.title}</h3>
                            <p className="text-xs text-slate-500 mt-1">총 {asg.problemIds.length}개 과제 문항</p>
                          </div>

                          {isDone ? (
                            <span className="px-3 py-1.5 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-lg text-xs font-bold">
                              제출 완료됨 ({submissions[subKey]?.score}점)
                            </span>
                          ) : (
                            <button
                              onClick={() => startAssignmentTest(asg.id, asg.problemIds)}
                              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow flex items-center space-x-1"
                            >
                              <span>과제 풀기</span>
              ...
              /* 학생 문제 풀이 및 디지털 터치 캔버스 뷰 */
              <div className="space-y-6">
                <div className="flex justify-between items-center bg-slate-900 p-4 rounded-2xl border border-slate-800 sticky top-20 z-40">
                  <div>
                    <h2 className="text-sm font-bold text-white">
                      {activeAssignmentId ? '[과제 수행]' : '[자율 맞춤 풀이]'} 총 {solvedProblems.length}문항 풀이 진행 중
                    </h2>
                    <p className="text-xs text-slate-400">화면 위의 터치 캔버스에 자유롭게 계산식 및 메모 필기를 진행하세요.</p>
                  </div>
                  ...
                  <div className="flex items-center space-x-2 bg-slate-950 p-1.5 rounded-xl border border-slate-800">
                    <button
                      ...
                      className={`p-2 rounded-lg text-xs flex items-center space-x-1 ${
                        activeTool === 'pen' ? 'bg-indigo-600 text-white' : 'text-slate-400'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg text-xs flex items-center space-x-1 ${
                        activeTool === 'eraser' ? 'bg-rose-600 text-white' : 'text-slate-400'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg text-xs flex items-center space-x-1 ${
                        activeTool === 'lasso' ? 'bg-blue-600 text-white' : 'text-slate-400'
                      }`}
                    >
                      <Lasso className="w-4 h-4" />
                      <span className="hidden sm:inline">올가미 선택/이동</span>
                    </button>
                  </div>
                </div>
                  ...
                  {solvedProblems.map((prob, idx) => (
                    <div key={prob.id} className="p-6 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl space-y-4">
                      <div className="flex justify-between items-center text-xs text-slate-400">
                        <span className="font-bold text-emerald-400 text-sm">문항 {idx + 1}</span>
                        <span>{prob.step1} &gt; {prob.step2} ({prob.difficulty})</span>
                      </div>

                      {prob.contentImage ? (
                        <img src={prob.contentImage} alt="문제" className="max-h-80 rounded" />
                      ) : (
                        <div className="text-sm text-slate-200 whitespace-pre-wrap">{prob.content}</div>
                      )}
                      ...
                      <div className="relative border border-slate-800 rounded-xl overflow-hidden bg-slate-950/80">
                        ...
                        />
                        {selectedStrokeIds.length > 0 && activeTool === 'lasso' && (
                          <button
                            onClick={() => deleteSelectedStrokes(prob.id)}
                            className="absolute bottom-2 right-2 px-3 py-1 bg-red-600 text-white text-xs rounded-lg shadow"
                          >
                            선택 필기 삭제
                          </button>
                        )}
                        ...
                        {prob.type === 'multiple' ? (
                          <div className="grid grid-cols-5 gap-2">
                            {numIcons.map((num, i) => {
                              ...
                              const isSelected = studentAnswers[prob.id] === val;
                              return (
                                <button
                                  key={i}
                                  onClick={() => setStudentAnswers({ ...studentAnswers, [prob.id]: val })}
                          ...
                          <input
                            type="text"
                            placeholder="주관식 정답 수치/단답을 입력하세요"
                      ...
                      {/* 풀이 결과 채점 및 상세 해설/유튜브 해설 보기 */}
                      {showResult && (
                        <div className="mt-4 p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-2 text-xs">
                          <div className="font-bold text-emerald-400">[정답]: {prob.answer}</div>
                          {prob.explanationImage ? (
                            <img src={prob.explanationImage} alt="해설" className="max-h-60 rounded border border-slate-800" />
                          ) : (
                            <div className="text-slate-300">[해설]: {prob.explanation}</div>
                          )}
                          {prob.youtubeUrl && (
                            <a href={prob.youtubeUrl} target="_blank" rel="noreferrer" className="text-red-400 underline block pt-1">
                              ▶ 문제 풀이 동영상 강의 보기
                            </a>
                          )}
                        </div>
                ...
                <div className="pt-4 flex gap-3">
                  <button
                    ...
                    className="flex-1 py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs shadow-lg"
                  >
                    최종 풀이 제출하기
                  </button>
                  <button
                    onClick={() => setStudentSolving(false)}
                    className="px-6 py-3.5 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold rounded-xl text-xs"
                  >
                    나가기
                  </button>
                </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-2xl w-full max-h-[85vh] overflow-y-auto space-y-4">
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                <h3 className="font-bold text-white text-sm">
                  [{viewingSubmission.studentName}] 학생 제출 결과
                </h3>
              ...
              <div className="text-xs space-y-1 text-slate-400">
                <p>과제: {viewingSubmission.assignmentTitle}</p>
                <p>제출 시각: {viewingSubmission.record.submittedAt}</p>
                <p className="text-emerald-400 font-bold">점수: {viewingSubmission.record.score}점</p>
              </div>
                ...
                {viewingSubmission.problems.map((prob, idx) => {
                  ...
                  return (
                    <div key={prob.id} className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-2 text-xs">
                      ...
                      {prob.contentImage ? (
                        <img src={prob.contentImage} alt="문제" className="max-h-40 rounded" />
                      ) : (
                        <p className="text-slate-300">{prob.content}</p>
                      )}

                      <div className="p-2 bg-slate-900 rounded border border-slate-800 space-y-1">
                        <p className="text-indigo-300">학생 답안: {studentAns}</p>
                        <p className="text-emerald-400">실제 정답: {prob.answer}</p>
                      </div>

                      {prob.explanationImage ? (
                        <img src={prob.explanationImage} alt="해설" className="max-h-48 rounded" />
                      ) : (
                        <p className="text-slate-400">[해설]: {prob.explanation}</p>
                      )}

                      {prob.youtubeUrl && (
                        <a href={prob.youtubeUrl} target="_blank" rel="noreferrer" className="text-red-400 underline block">
                          유튜브 해설 보기
                        </a>
                      )}
                    </div>
        ...
        {editingProblem && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-xl w-full max-h-[85vh] overflow-y-auto space-y-4">
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                <h3 className="font-bold text-white text-sm">문제 정보 수정</h3>
              ...
              <form onSubmit={handleUpdateProblem} className="space-y-4 text-xs">
                <div>
                  <label className="block text-slate-400 mb-1">제목</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-slate-400 mb-1">문제 지문 텍스트</label>
                  <textarea
                    rows={3}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-slate-400 mb-1">정답</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-slate-400 mb-1">난이도</label>
                    <select
                ...
                <div>
                  <label className="block text-slate-400 mb-1">상세 해설 텍스트</label>
                  <textarea
                    rows={3}
                ...
                <div>
                  <label className="block text-slate-400 mb-1">유튜브 해설 링크</label>
                  <input
                    type="text"
                ...
                <div className="flex gap-2 pt-2">
                  <button type="submit" className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg">
                    수정 완료 저장
                  </button>
                  <button
                    type="button"
